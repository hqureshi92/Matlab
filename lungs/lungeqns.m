function dy = lungeqns(t, y, alpha, fdelp, fdelm, Kpgp, V1, Ve, P1S1, P2S2, kbar1, kinv, F, Vr)
...
%     Vr    = Reservoir volume                        [mL]
    ...
    V2 = 0.02*V1; %Apparent mitochondrial matrix vol. [mL]
    %Eq. 1 (d[Re]/dt)
    dy(1) = (Kpgp/Ve)*y(2) - (P1S1*J1)/Ve - kbar1*y(1) + kinv*y(4) + (F/Ve)*(y(5) - y(1));
    %Eq. 2 (d[Rc]/dt)
    dy(2) = -(Kpgp/V1)*y(2) + (P1S1*J1)/V1 - (P2S2*J2)/V1;
    %Eq. 3 (d[Rm]/dt)
    dy(3) = (P2S2*J2)/V2;
    %Eq. 4 (d[ReBe]/dt)
    dy(4) = kbar1*y(1) - kinv*y(4);
    %Eq. 10 (d[C]/dt)
    dy(5) = (F/Vr)*(y(1) - y(5));